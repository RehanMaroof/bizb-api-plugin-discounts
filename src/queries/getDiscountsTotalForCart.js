...
export default async function getDiscountsTotalForCart(context, cart) {
  ...
  let discounts = await Promise.all(
    (cart.billing || []).map(async (billing) => {
      ...
      const calculation =
        discount && discount.calculation && discount.calculation.method;
      if (!calculation) return null;
      console.log("discount.discountMethod", discount.discountMethod)
      if (discount?.discountMethod === 'amount') {
        var funcs = context.getFunctionsOfType(
          `discounts/amounts/${calculation}`
        ); // note the added "s"
        if (funcs.length === 0)
          throw new Error(
            `No functions of type "discounts/amounts/${calculation}" have been registered`
          );
      } else {
        var funcs = context.getFunctionsOfType(
          `discounts/${processor}s/${calculation}`
        ); // note the added "s"
        if (funcs.length === 0)
          throw new Error(
            `No functions of type "discounts/${processor}s/${calculation}" have been registered`
          );
      }
  ...
  console.log("test console", discounts);
